...
// Language categories based on SF2e/PF2e core rules
const languageCategories = {
  common: {
    label: 'Common Languages',
    items: [
      'Common', 'Draconic', 'Dwarven', 'Elven', 'Gnomish', 'Goblin',
      'Halfling', 'Orcish', 'Akitonian', 'Vesk', 'Ysoki'
    ]
  },
  uncommon: {
    label: 'Uncommon Languages',
    items: [
      'Aklo', 'Alghollthu', 'Androffan', 'Azlanti', 'Jotun', 'Necril',
      'Shadowtongue', 'Undercommon', 'Vercite', 'Kasathan', 'Shirren'
    ]
  },
  monster: {
    label: 'Monster Languages',
    items: [
      'Empyrean', 'Chthonian', 'Diabolic', 'Daemonic', 'Protean', 'Requian',
      'Utopian', 'Fey', 'Destrachan', 'Formian', 'Grioth', 'Nuar'
    ]
  },
  elemental: {
    label: 'Elemental Languages',
    items: [
      'Sussuran', 'Petran', 'Pyric', 'Talican', 'Thalassic', 'Muan'
    ]
  },
  tech: {
    label: 'Tech & Construct',
    items: [
      'Trinary', 'Aballonian', 'Machine'
    ]
  }
}
...
function addCustom() {
  const item = customLanguage.value.trim()
...
const knownItems = computed(() => {
  ...
  for (const cat of Object.values(languageCategories)) {
      ...
      <div v-if="isOpen" class="picker-overlay" @click="isOpen = false">
        <div class="picker-popover" @click.stop>
          <div class="popover-header">
            <span class="popover-title">Select Languages</span>
            <button type="button" class="btn-close" @click="isOpen = false">×</button>
          </div>

          <div class="popover-body">
            <div
              v-for="(category, key) in languageCategories"
              :key="key"
              class="item-category"
            >
