...
const customImmunity = ref('')

// Immunity categories based on GM Core creature building rules
const immunityCategories = {
  conditions: {
    label: 'Conditions',
    items: [
      'bleed', 'confused', 'controlled', 'death effects', 'disease',
      'doomed', 'drained', 'fatigued', 'frightened', 'grabbed',
      'healing', 'nonlethal', 'paralyzed', 'persistent damage',
      'poison', 'prone', 'restrained', 'sickened', 'sleep',
      'stunned', 'unconscious'
    ]
  },
  damage: {
    ...
    items: [
      ...
      'force', 'mental', 'poison', 'precision',
      'spirit', 'vitality', 'void', 'holy', 'unholy'
    ]
  },
  special: {
    label: 'Special',
    items: [
      'critical hits', 'fear', 'magic', 'swarm mind'
    ]
  }
}
...
function addCustom() {
  const item = customImmunity.value.trim()
  if (item && !model.value.includes(item)) {
    model.value = [...model.value, item]
  }
  customImmunity.value = ''
}

// Check if an item is known
const knownItems = computed(() => {
  const all = new Set<string>()
  for (const cat of Object.values(immunityCategories)) {
      ...
      <div v-if="isOpen" class="picker-overlay" @click="isOpen = false">
        <div class="picker-popover" @click.stop>
          <div class="popover-header">
            <span class="popover-title">Select Immunities</span>
            <button type="button" class="btn-close" @click="isOpen = false">×</button>
          </div>

          <div class="popover-body">
            <div
              v-for="(category, key) in immunityCategories"
              :key="key"
              class="item-category"
            >
